...
ログインが必要な検証では、**既存ユーザー（開発者が作成したアカウント）には一切触れず、必ず使い捨ての新規テストユーザーを作成して使う**。

- ログイン画面から新規登録するか、`rails runner`等で検証専用のユーザーを作成する（例: `evaluator_test+{timestamp}@example.com`のような一意なメールアドレスを使う）
- 既存ユーザーのパスワードを変更・リセットして流用することは禁止（元のパスワードが分からなくなり、開発者がログインできなくなる事故につながる）
- 既存ユーザーのデータ（試合分析、スコア等）を書き換えたり削除したりしない
- 検証で作成したテストユーザーとそのデータは、検証終了後に必ず削除する
- どうしても既存ユーザーでのログイン確認が必要な場合は、検証を進める前に必ずユーザー（人間）に確認を取る
